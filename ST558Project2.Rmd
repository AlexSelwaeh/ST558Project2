...
#final model: fit4: lm(cnt ~ season + I(yr)^2 + weathersit + temp + atemp, data = train)
#check the fit
lmPred <- predict(fit4, newdata = test)
lmfit <- postResample(lmPred, obs = test$cnt)
lmfit[1]

#fitting boosted tree model for counts
set.seed(1)
train$yrsquare <- (train$yr)^2
test$yrsquare <- (test$yr)^2
boostFit <- gbm(cnt ~ season + yrsquare + weathersit + atemp + temp, data = train, 
